...
void AccountSelectPage::on_pushButton_addNew_clicked()
{
    ...
    const QString imagePath = pickRandomImagePath();
    const QSize  btnSize    = accountBtn->size();
    const int    radius     = 15;

    QPixmap rounded = makeRoundedCover(imagePath, btnSize, radius);
    ...
    layout->insertWidget(insertIndex, container);
                                                    ...
                                                    const QSize& btnSize, const QString& imagePath)
{
    ...
    closeBtn->setText(QString::fromUtf8("✕")); // 아이콘 파일 있으면 setIcon 사용
...
// DB 기준으로 모든 계정 버튼을 다시 그린다. (앱 재실행 복원의 핵심)
