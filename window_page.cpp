...
void initWindowConnections(MainWindow* w)
{
    ...
    QObject::connect(ui->stackedWidget, &QStackedWidget::currentChanged, [=](int index) {
        if (ui->stackedWidget->widget(index) == ui->page_window) {
            if (windowOpenMovie) {
...
void onWindowOpenPressed(MainWindow* w)
{
    ...
    // TODO: FIFO 전송
    // QString json = R"({"device":"window", "command":"press", "value":"open"})";
...
void onWindowOpenReleased(MainWindow* w)
{
    ...
    // TODO: FIFO 전송
    // QString json = R"({"device":"window", "command":"release", "value":"open"})";
    // w->sendJsonToFifo(json);
}
...
void onWindowClosePressed(MainWindow* w)
{
    ...
    // TODO: FIFO 전송
    // QString json = R"({"device":"window", "command":"press", "value":"close"})";
    // w->sendJsonToFifo(json);
}
...
void onWindowCloseReleased(MainWindow* w)
{
    ...
    // TODO: FIFO 전송
    // QString json = R"({"device":"window", "command":"release", "value":"close"})";
    // w->sendJsonToFifo(json);
}
