...
void initAirconConnections(MainWindow* w)
{
    ...
    QObject::connect(w->getUi()->pushButton_aircon_on, &QPushButton::clicked, [=]() {
        if (auto banner = w->findChild<NotificationBanner*>("NotificationBanner")) {
//            banner->setAccentColor(QColor("#3cf0ff")); // 좋아보이는 청록 라인
            banner->setMessage(QStringLiteral("에어컨 전원 ON (LOW)"));
            banner->showBanner(3000);
        }
        ...
        /*
        if (!shm_ptr) return;
        shm_ptr->aircon.level = 1;
        */
    });

    QObject::connect(w->getUi()->pushButton_aircon_off, &QPushButton::clicked, [=]() {
        if (auto banner = w->findChild<NotificationBanner*>("NotificationBanner")) {
//            banner->setAccentColor(QColor("#ff5555")); // 경고계열
            banner->setMessage(QStringLiteral("에어컨 전원 OFF"));
            banner->showBanner(3000);
        }
